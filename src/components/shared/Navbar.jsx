...
const Navbar = () => {
  ...
  const userRole = user?.role || 'user'; // Extract user role
  ...
  // 2. Role-based links
  if (userRole === 'admin') {
    // If Admin, show Management links
    navLinks.push({
      name: 'Manage Lessons',
    ...
    // If User (or Guest), show Standard links
  ...
  if (isLoggedIn && userRole === 'user' && !isPremium) {
            ...
            <div className="relative" ref={dropdownRef}>
              ...
              {isDropdownOpen && (
                <div className="absolute right-0 mt-4 w-56 bg-[#111111] border border-white/10 rounded-xl shadow-2xl py-3 z-50 animate-in fade-in zoom-in duration-200">
                  ...
                  {/* Common Dashboard/Profile Links */}
                  <Link
                    href={
                      userRole === 'admin'
